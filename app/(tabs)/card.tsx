...
/**
 * The active card: look at it, or show its QR.
 *
 * Editing lives at `/card/edit`, not here. This screen used to carry its own
 * inline editor — a stacked form plus a sticker tray — and for a while the app
 * had two, which is how a card could be styled one way in one place and another
 * way in the other. The editor screen absorbed the sticker work and this one
 * became the display it was always trying to be.
 */
export default function CardScreen() {
	...
	// The editor is pushed over this screen, so coming back fires no mount. Without
	// this the card would still be drawn as it looked on the way in.
	...
	useEffect(() => {
		...
		Promise.all([
			...
			updateCard({
				...
				links: Array.isArray(row.links) ? (row.links as unknown as CardLink[]) : [],
				stickers: (stickerResult.data ?? []) as PlacedSticker[]
			});
		});
	}, [profile, updateCard, refreshKey]);
		...
		>
			...
			{mode === 'card' ? (
				<View style={styles.viewActions}>
					<Button label="Edit this card" onPress={() => router.push('/card/edit')} />
